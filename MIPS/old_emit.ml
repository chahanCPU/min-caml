...
and g' oc = function (* 各命令のアセンブリ生成 (caml2html: emit_gprime) *)
  ...
  | NonTail(_), Nop -> ()
  | NonTail(x), Li(i) when -32768 <= i && i < 32768 -> Printf.fprintf oc "\tori\t%s, $zero, %d\n" (reg x) i
  | NonTail(x), Li(i) ->
      let n = i lsr 16 in
  ...
  | Tail, IfEq(x, V(y), e1, e2) ->
      Printf.fprintf oc "\txor\t$s7, %s, %s\n" (reg x) (reg y);
      g'_tail_if oc e1 e2 "beq" "bne"
  | Tail, IfEq(x, C(y), e1, e2) ->
      Printf.fprintf oc "\txori\t$s7, %s, %d\n" (reg x) y;
      g'_tail_if oc e1 e2 "beq" "bne"
  | Tail, IfLE(x, V(y), e1, e2) ->
      Printf.fprintf oc "\tori\t$at, $zero, -1\n"; 
      Printf.fprintf oc "\tmult\t%s, $at, %s\n" (reg x) (reg x); 
      Printf.fprintf oc "\tmult\t%s, $at, %s\n" (reg y) (reg y); 
      Printf.fprintf oc "\tslt\t$s7, %s, %s\n" (reg x) (reg y);
      g'_tail_if oc e1 e2 "beq" "bne"
  | Tail, IfLE(x, C(y), e1, e2) ->
      Printf.fprintf oc "\tori\t$at, $zero, -1\n"; 
      Printf.fprintf oc "\tmult\t%s, $at, %s\n" (reg x) (reg x); 
      Printf.fprintf oc "\tslti\t$s7, %s, %d\n" (reg x) (-y);
      g'_tail_if oc e1 e2 "beq" "bne"
  ...
  (* | Tail, IfFEq(x, y, e1, e2) ->
      Printf.fprintf oc "\tfcmpu\t$s7, %s, %s\n" (reg x) (reg y);
      g'_tail_if oc e1 e2 "beq" "bne"
  ...
  | NonTail(a), CallCls(x, ys, zs) ->
      Printf.fprintf oc "\tor\t%s, $ra, $zero\n" (reg reg_tmp);
      g'_args oc [(x, reg_cl)] ys zs;
      let ss = stacksize () in
      ...
      Printf.fprintf oc "\tor\t$ra, %s, $zero\n" (reg reg_tmp)
  | (NonTail(a), CallDir(Id.L(x), ys, zs)) ->
      ...
      Printf.fprintf oc "\tlw\t%s, %d(%s)\n" (reg reg_tmp) (ss - 4) (reg reg_sp);
      if List.mem a allregs && a <> regs.(0) then
        Printf.fprintf oc "\tor\t%s, %s, $zero\n" (reg a) (reg regs.(0))
      else if List.mem a allfregs && a <> fregs.(0) then
        Printf.fprintf oc "\tfmr\t%s, %s\n" (reg a) (reg fregs.(0));
      Printf.fprintf oc "\tor\t$ra, %s, $zero\n" (reg reg_tmp)
and g'_tail_if oc e1 e2 b bn =
  let b_else = Id.genid (b ^ "_else") in
  Printf.fprintf oc "\t%s\t$s7, $zero, %s\n" bn b_else;
...
and g'_args oc x_reg_cl ys zs =
  let (i, yrs) =
    List.fold_left
      ...
      ys in
  List.iter
    (fun (y, r) -> Printf.fprintf oc "\tor\t%s, %s, $zero\n" (reg r) (reg y))
    (shuffle reg_sw yrs) (* ;f *)
  (* let (d, zfrs) =
    List.fold_left
